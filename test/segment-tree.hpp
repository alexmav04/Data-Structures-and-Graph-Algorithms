...
private:
  ...
  void AllocateEmptyST(
      const std::vector<T> &);           // Allocate the space for an empty ST.
  void DeallocateST();                   // Delete the ST previously created.
  void FillInST(const std::vector<T> &); // Fill the ST with the proper values.
  T RecursivelyFillST(
      int, int, int,
      const std::vector<T> &); // Recursively find, set, return min of node.
  T RecursivelySearchForMin(int, int, int, int,
                            int); // Recursively search a node from L to R for
                                  // it's children's min values (if necessary).
  void RecursivelyUpdate(int, T, int, int,
                         int); // Recursively update a node with the newValue if
                               // it's within the node's range.
...
template <class T>
T SegmentTree<T>::RecursivelyFillST(int nodeIndex, int nodeStartIndex,
                                    int nodeEndIndex,
  ...
  // Otherwise, we need to find the min by recursively asking this node's
  // children for their mins. Calculate the start and end indexes for the two
  // children.
  ...
  T leftChildMin = RecursivelyFillST(leftChildNodeIndex, leftChildStartIndex,
                                     leftChildEndIndex, originalArray);

  T rightChildMin = RecursivelyFillST(rightChildNodeIndex, rightChildStartIndex,
                                      rightChildEndIndex, originalArray);
...
void SegmentTree<T>::FillInST(const std::vector<T> &originalArray) {
  // Starting with the root node of the st, recursively fill in the values in
  // the ST.
  int rootNodeIndex = 0;
  int rootNodeStartIndex = 0;
  int rootNodeEndIndex = originalArrayLength - 1;
  RecursivelyFillST(rootNodeIndex, rootNodeStartIndex, rootNodeEndIndex,
                    originalArray);
}

template <class T> SegmentTree<T>::SegmentTree(std::vector<T> originalArray) {
...
////////////////////////////////////////////////////////////
/////////// Destructor /////////////////////////////////////
template <class T> void SegmentTree<T>::DeallocateST() {
  // Delete the st array, set the pointer to null, and internally say the length
  // is 0.
  delete[] st;
  st = nullptr;
  stArrayLength = 0;
...
template <class T> SegmentTree<T>::~SegmentTree() { DeallocateST(); }
////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////
...
/////////// Query //////////////////////////////////////////
template <class T>
T SegmentTree<T>::RecursivelySearchForMin(int L, int R, int nodeIndex,
                                          int nodeStartIndex,
                                          int nodeEndIndex) {
  ...
  // Otherwise, if this node is completely within [L, R], just return the node's
  // value.
  ...
  // Otherwise, this node is partially within [L, R].
    ...
    // Recursively check this node's children and return the min between the
    // two.
    int middleIndex = nodeStartIndex + ((nodeEndIndex - nodeStartIndex) / 2);
    ...
    T leftChildMin = RecursivelySearchForMin(
        L, R, leftChildNodeIndex, leftChildStartIndex, leftChildEndIndex);

    T rightChildMin = RecursivelySearchForMin(
        L, R, rightChildNodeIndex, rightChildStartIndex, rightChildEndIndex);
...
template <class T> T SegmentTree<T>::query(int L, int R) {
  ...
  // Starting with the root, recursively search the tree for it's min value in
  // [L, R].
  int rootNodeIndex = 0;
  int rootNodeStartIndex = 0;
  int rootNodeEndIndex = originalArrayLength - 1;
  T value = RecursivelySearchForMin(L, R, rootNodeIndex, rootNodeStartIndex,
                                    rootNodeEndIndex);
  return value;
}
////////////////////////////////////////////////////////////
...
/////////// Update //////////////////////////////////////////
template <class T>
void SegmentTree<T>::RecursivelyUpdate(int newValueIndex, T newValue,
                                       int nodeIndex, int nodeStartIndex,
                                       int nodeEndIndex) {
  // If the start index is the same as the end index, then this range represents
  // one node. We'll also check that this node's start/endIndex are the same as
  // the newValueIndex. The exact value of this node must be updated with the
  // new value.
  ...
  // We only need to update one subtree, either left or right, based on the
  // newValueIndex. If the newValueIndex is in the left subtree...
  if (nodeStartIndex <= newValueIndex && newValueIndex <= middleIndex) {
    // ...Then recursively update the left children as necessary.
    int leftChildStartIndex = nodeStartIndex;
    int leftChildEndIndex = middleIndex;

    RecursivelyUpdate(newValueIndex, newValue, leftChildNodeIndex,
                      leftChildStartIndex, leftChildEndIndex);
  }
  // Otherwise if the newValueIndex is in the right subtree...
    ...
    RecursivelyUpdate(newValueIndex, newValue, rightChildNodeIndex,
                      rightChildStartIndex, rightChildEndIndex);
  }
...
template <class T> void SegmentTree<T>::update(int newValueIndex, T newValue) {
  std::cout << "Updating value at index " << newValueIndex
            << " to new value of " << newValue << ".\n";
  ...
  // Starting with the root, recursively update the nodes if the newValue is
  // better than the node's current min (and the index is in the node's range).
  int rootNodeIndex = 0;
  int rootNodeStartIndex = 0;
  int rootNodeEndIndex = originalArrayLength - 1;
  RecursivelyUpdate(newValueIndex, newValue, rootNodeIndex, rootNodeStartIndex,
                    rootNodeEndIndex);
}
////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////
